<template>
  <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen font-sans">
    <div class="lg:col-span-3">
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800">Hi, {{ candidate.name.split(' ')[0] }} 👋</h1>
        <p class="text-gray-500">Here's your job search summary for today.</p>
      </div>

      <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-700">Profile Completeness</h2>
        ...
        <div class="w-full bg-gray-200 rounded-full h-2.5">
            <div class="bg-indigo-600 h-2.5 rounded-full" :style="{ width: `${profileCompleteness}%` }"></div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-md">
        ...
        <ul class="divide-y divide-gray-200">
            <li v-for="app in applications" :key="app.id" class="p-6 hover:bg-gray-50 transition-colors duration-200">
      ...
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="space-y-4">
            <div v-for="job in recommendedJobs" :key="job.id" class="p-4 rounded-lg hover:bg-gray-50 border border-gray-200 transition-colors duration-200">
                ...
                <button class="w-full text-center mt-3 bg-primary text-white py-2 rounded-lg text-sm font-semibold hover:bg-indigo-600">
                    View Details
                </button>
            </div>
        </div>
